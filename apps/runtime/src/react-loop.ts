...
export interface ReactLoopCallbacks {
  ...
  thinkStream?: (context: Message[]) => AsyncIterable<string>;
...
export class ReactLoop {
  ...
  async *runStream(): AsyncGenerator<string, { output: string; messages: Message[] }, unknown> {
    if (!this.callbacks.thinkStream) {
    ...
    // 实时 yield token
    for await (const chunk of this.callbacks.thinkStream(context)) {
      yield chunk;
      fullResponse += chunk;
    }

    const toolCall = this.callbacks.parseToolCall(fullResponse);
    ...
    if (toolCall) {
      ...
      this.stepCount++;
      // 递归调用，继续 yield 后续的流
      return yield* this.runStream();
    } else {
      return { output: fullResponse, messages: this.messages };
    }
  }
